...
from keras.layers import Input, Dense, Lambda, Activation, Conv1D, Dropout, LeakyReLU, Reshape
from keras.layers import dot, multiply, concatenate, add
...
class shan():
    ...
    def build_model(self):
        ...
        s_i = []
        for i in range(self.max_num_sent):
            h_i_t = Lambda(lambda xin: xin[:, i, :])(encoded_article)
            s_i_t = Lambda(
                lambda xin: K.expand_dims(
                    xin, axis=1))(
                self.personalized_attention(
                    u_w, h_i_t))
            s_i.append(s_i_t)
        s_i = concatenate(s_i, axis=1)
        """
