...
"""
# global variables
activity_name = '../processed_user_activity.json'
relation_name = '../processed_user_relation.json'
index_name = '../processed_user_index.json'
article_name = '../new_article.json'
...
user_reading = {}
new_article_dict = {}
day_count = 127  # 2017-7-1 - 2018-7-6
user_payout = {}
daily_nx = {}
user_payout_all = {}
...
def parse_args():
    ...
    parser.add_argument('--word-representation', nargs='?', default='../word2vec.txt',
...
def build_social_graph():
    ...
    np.savetxt('../karate.edgelist', relation_list, fmt='%d')
...
def build_sample():
    ...
    """
    with open(activity_name, "r", encoding="utf-8-sig") as f:
...
def create_vocab():
    ...
    for key in article_dict:
        ...
        progress += 1
        sleep(0.00001)
        article_dict[key]['text'] = update_vocab(article_dict[key]['text'])
    bar.finish()
...
def sample_format(args, vocab):
    ...
    while i < day_count-6:
        week_id = j + 1
        social_sample_data[week_id] = []
        user_payout_all[week_id] = {}
        for k in range(7):
            day_id = i + 1
            for user_name in user_reading:
                if day_id not in user_reading[user_name].keys():
                ...
                if k == 6:
                    user_payout_all[week_id][user_id] = 0
                    for u_day_id in range(1, day_id):
                        if u_day_id in user_payout[user_name]:
                            user_payout_all[week_id][user_id] += sum(user_payout[user_name][u_day_id])
                    ...
                    for article_id in user_reading[user_name][day_id]["positive"]:
                        if article_id in new_article_dict:
                            social_sample = [
                                ...
                                [1]]
                            social_sample_data[week_id].append(social_sample)
        ...
        j += 1
        # padding
        d_vectorized_sample = np.array(social_sample_data[week_id])
        ...
        vectorized_data[week_id] = [d_user_index, d_article, d_label]
        out_file_user = args.feed_output + 'week_user_' + str(week_id)
        out_file_article = args.feed_output + 'week_article_' + str(week_id)
        out_file_creator = args.feed_output + 'week_creator_' + str(week_id)
        out_file_label = args.feed_output + 'week_label_' + str(week_id)
...
def get_embed(vocab, args):
    ...
    for word, i in vocab.items():
        ...
        if embedding_vector is not None:
            ...
            emb_matrix[i] = np.random.uniform(-0.25, 0.25, opt.embedding_dim)
